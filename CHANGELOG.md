...
- **Breaking:** every ledger query takes `--account <alias|r-address>` instead of `--address`. `--address` remains as a hidden alias for one minor. `--account` **refuses anything that parses as a seed**, because at worst a seed there matches a stored alias and signs with the wrong key. Resolution is `--account` → `XRPL_ACCOUNT` → the configured default, with no project-local tier; anything not named explicitly is announced on stderr, and an inherited account is refused on mainnet.
- **`xrpl tx new` resolves aliases in every address field.** `--destination bob`, `--holder bob`, `--signer-entry bob:1`, `--amount 100/USD/bob` and `--field RegularKey=bob` all take a recorded account's address, for every field the definitions type as `AccountID`. A valid address is always literal; any other value must name an account record, and an unknown name exits 1 naming its flag. The lookup reads the address only — no key, no unlock, no network — and only `--account` falls back to `XRPL_ACCOUNT` and the configured default.
