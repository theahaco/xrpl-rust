...
- `xrpl account fund <alias-or-address>` requests Testnet or Devnet XRP for an existing account without unlocking a key. Supports a custom `--faucet-url`, waits for a validated balance increase, and bounds the request and polling with `--timeout`. Requires an explicit network or node URL and sends the faucet request only once.
