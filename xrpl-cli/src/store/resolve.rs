//! Turning `--account` into an address.
//!
...
//!
//! Alias resolution is confined to this one flag. `--destination`, `--issuer`,
//! `--holder`, `--delegate` and the address half of `--signer-entry` take
//! literal addresses forever. The alternative is the union type stellar-cli
//! ended up with, where one argument could be an alias *or* a secret key, and a
//! literal address silently could not sign.
//!
//! # The ladder
//!
...
fn reject_secret(value: &str) -> Result<(), Error> {
    ...
    if looks_like_seed {
        return Err(Error::other(
            "--account was given something that looks like a seed. It names an \
